...
def upload_file():
    """Open a file dialog to select the preprocessed OTT dataset."""
...
# Load the preprocessed OTT dataset
df = pd.read_csv(file_path)
print("\n📂 Preprocessed OTT Dataset Loaded!\n")

# Define Features (X) and Target (y)
X = df.drop(columns=['churn', 'customer_id', 'phone_no', 'year'])  # Remove non-relevant columns
...
# Plot 2D LDA Visualization
ax3 = fig.add_subplot(133)
ax3.scatter(X_lda[:, 0], [0] * len(X_lda), c=y, cmap='coolwarm', alpha=0.7)
ax3.set_title("2D LDA Visualization")
ax3.set_xlabel("LDA1")
ax3.set_yticks([])  # Hide Y-axis as it's 1D

plt.show(block=True)  # Force plots to display
